(function () {
  ...
  function renderPrivilegeInfo() {
    return `<section class="card p-5">
      <h3 class="text-xl font-black">Control de privilegios</h3>
      ...
      <div class="grid md:grid-cols-2 gap-4 mt-3">
        <article class="prize-place">
          <h4>Un usuario ROOT puede:</h4>
          <ul class="list-decimal ml-5 text-sm text-slate-700">
            <li>Otorgar permisos de administrador.</li>
            <li>Revocar permisos de administrador.</li>
            <li>Designar o reemplazar administradores operativos.</li>
          </ul>
        </article>

        <article class="prize-place">
          <h4>Administradores operativos</h4>
          <p class="text-sm text-slate-700">Pueden gestionar torneo, pagos, resultados, especiales y configuración, pero no pueden crear, modificar ni eliminar usuarios ROOT.</p>
        </article>
      </div>
    </section>`;
  }
  ...
  function renderRolesMigrationInfo() {
    return `<section class="card p-5">
      <h3 class="text-xl font-black">Migración de roles y mantenimiento</h3>
      ...
      <div class="grid md:grid-cols-4 gap-3 mt-3">
        <article class="prize-place">
          ...
          <p class="text-sm">Agrega roles ROOT/ADMIN/USER y procedimientos de mantenimiento del torneo.</p>
        </article>

        <article class="prize-place">
          <h4>Cuándo ejecutarla</h4>
          <p class="text-sm">Después de tener la estructura base y antes de usar mantenimiento o control de roles.</p>
        </article>

        <article class="prize-place">
          <h4>Qué agrega</h4>
          <p class="text-sm">Columna <code>profiles.role</code>, helpers de permisos, <code>set_profile_role()</code>, <code>reset_test_data()</code> y <code>reset_tournament_results()</code>.</p>
        </article>

        <article class="prize-place">
          <h4>Qué NO hace</h4>
          <p class="text-sm">No crea usuarios, no carga seeds, no borra equipos, partidos, jugadores, torneo ni configuración.</p>
        </article>
      </div>
  ...
  function renderAutomationSettings() {
    ...
    return `<section class="card p-5">
      ...
      <div class="flex gap-2 mt-3">
        <button class="btn btn-secondary" onclick="PronostixAdmin.saveAutomationSettings()">Guardar preparación API</button>
        <button class="btn btn-secondary" disabled>Sincronizar resultados (próximamente)</button>
      </div>
  ...
  function renderMaintenance() {
    return `<section class="card p-5">
      <h3 class="text-xl font-black">Mantenimiento del torneo</h3>
      ...
      <div class="grid md:grid-cols-2 gap-4 mt-3">
        <article class="prize-place">
          ...
          <p class="text-sm text-slate-700">Limpia todo dato generado por pruebas para dejar el torneo listo para iniciar desde cero.</p>
          <p class="text-sm"><b>Borra:</b> pronósticos, pronósticos especiales, resultados especiales y resultados capturados en partidos.</p>
          <p class="text-sm"><b>Conserva:</b> usuarios/profiles, pagos, roles, torneos, configuración, equipos, partidos y jugadores.</p>
          <button class="btn btn-secondary mt-3" onclick="PronostixAdmin.resetTestData()">Limpiar datos de prueba</button>
        </article>

        <article class="prize-place">
          <h4>Limpiar resultados</h4>
          <p class="text-sm text-slate-700">Limpia SOLO resultados para volver a simular el torneo sin borrar pronósticos.</p>
          <p class="text-sm"><b>Borra:</b> resultados especiales y resultados capturados en partidos.</p>
          <p class="text-sm"><b>Conserva:</b> pronósticos, pronósticos especiales, usuarios/profiles, pagos, roles, torneos, configuración, equipos, partidos y jugadores.</p>
          <button class="btn btn-secondary mt-3" onclick="PronostixAdmin.resetTournamentResults()">Limpiar resultados</button>
        </article>
      </div>
    </section>`;
  }
  ...
  function renderDataLoadInfo() {
    return `<section class="card p-5">
      <h3 class="text-xl font-black">Carga de datos base</h3>
      ...
      <ol class="list-decimal ml-5 mt-2 text-slate-700">
        <li><code>sql/seed_worldcup_2026.sql</code></li>
        <li><code>sql/seed_worldcup_2026_players_candidates.sql</code></li>
        <li><code>sql/validate_worldcup_2026_seed.sql</code></li>
      </ol>

      <div class="grid md:grid-cols-4 gap-3 mt-3">
        <article class="prize-place"><h4>Schema</h4><p class="text-sm">Crea estructura desde cero. Úsalo solo para bases nuevas.</p></article>
        <article class="prize-place"><h4>Migraciones</h4><p class="text-sm">Actualizan una base existente sin recrearla.</p></article>
        <article class="prize-place"><h4>Seeds</h4><p class="text-sm">Cargan datos iniciales o de producción.</p></article>
        <article class="prize-place"><h4>Reset</h4><p class="text-sm">Limpia pruebas sin perder calendario.</p></article>
      </div>
    </section>`;
  }
  ...
  async function resetTestData() {
    if (window.prompt("Escribe REINICIAR PRUEBAS para confirmar.") !== "REINICIAR PRUEBAS") {
      return P.toast("Operación cancelada.", false);
    }
  ...
  async function resetTournamentResults() {
    if (window.prompt("Escribe LIMPIAR RESULTADOS para confirmar.") !== "LIMPIAR RESULTADOS") {
      return P.toast("Operación cancelada.", false);
    }
